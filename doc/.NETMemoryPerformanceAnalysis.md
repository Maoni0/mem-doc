...
Network IO is just an example of another factor that could contribute to your request latency. And boxe widths here are merely for illustration purpose. 
...
For each of these processes, you will get the following details:
...
Double clicking on the name of an event shows all the occurrences of that event. In the process, it loads all events in which you can search for specific details. For example, if I want to find out the exact version of the coreclr.dll I can type **coreclr** in the Find textbox:

<img src=".\FileVer-coreclr.jpg" alt="demotion" style="zoom:80%;" />

Then you can see the exact version info of the coreclr.dll you are using. Remember, if you don't load the events you are interested in, the search will fail.
